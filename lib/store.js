...
export function ensureSeed() {
  ...
  // পুরনো ডেমো "স্বাগতম" নোটিফিকেশন থাকলে সরান (fake cleanup):
  try {
    const n = read(K.notif, []);
    const clean = n.filter(x => x.title !== '🔔 স্বাগতম!');
    if (clean.length !== n.length) write(K.notif, clean);
  } catch {}
  if (!localStorage.getItem(K.notif)) write(K.notif, []);
