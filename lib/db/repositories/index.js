...
const messaging = {
  listByUser: async (userId, limit = 20) => (await query(
    `SELECT c.*, (SELECT m2.message FROM messages m2 WHERE m2.conversation_id=c.id ORDER BY m2.created_at DESC LIMIT 1) AS last_message,
       (SELECT m2.created_at FROM messages m2 WHERE m2.conversation_id=c.id ORDER BY m2.created_at DESC LIMIT 1) AS last_at,
       (SELECT u.full_name FROM conversation_members m2 JOIN users u ON u.id=m2.user_id WHERE m2.conversation_id=c.id AND m2.user_id != $1 LIMIT 1) AS other_name
     FROM conversations c JOIN conversation_members m ON m.conversation_id=c.id
     WHERE m.user_id=$1 ORDER BY last_at DESC NULLS LAST, c.created_at DESC LIMIT $2`, [userId, Math.min(limit, 50)])).rows,
  members: async (conversationId) => (await query(
    `SELECT user_id FROM conversation_members WHERE conversation_id=$1`, [conversationId])).rows.map(r => r.user_id),
  ...
  listMessages: async (conversationId, { cursor, limit = 20 } = {}) => {
    ...
    const ks = keysetClause(params, cursor, 'DESC'); w += ks.clause;
    const r = await query(`SELECT t.*, (SELECT u.full_name FROM users u WHERE u.id=t.sender_id) AS sender_name FROM messages t WHERE ${w} ORDER BY t.created_at DESC, t.id DESC LIMIT $${ks.params.length + 1}`, ks.params.concat(limit + 1));
    return pageOf(r.rows, limit);
  }
};
