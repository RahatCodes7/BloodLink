...
/** Owner-এর সব অনুরোধ (management: ACTIVE + FULFILLED + CANCELLED সহ)। */
async function listByRequester(requesterId, limit = 50) {
  limit = Math.min(Math.max(limit, 1), 50);
  const r = await query(
    `SELECT * FROM blood_requests t WHERE t.requester_id=$1 AND t.deleted_at IS NULL ORDER BY t.created_at DESC LIMIT $2`,
    [requesterId, limit]
  );
  return r.rows;
}

/** অ্যাডমিন: সব স্ট্যাটাসের অনুরোধ (MOD রোল API-তে যাচাই হয়)। */
async function listAllForAdmin({ status, limit = 50, offset = 0 } = {}) {
  limit = Math.min(Math.max(limit, 1), 50);
  const params = []; let w = 't.deleted_at IS NULL';
  if (status) { params.push(status); w += ` AND t.status=$${params.length}`; }
  params.push(limit, Math.max(offset, 0));
  const r = await query(
    `SELECT t.*, h.name_bn AS hospital_name FROM blood_requests t LEFT JOIN hospitals h ON h.id=t.hospital_id
     WHERE ${w} ORDER BY t.created_at DESC LIMIT $${params.length - 1} OFFSET $${params.length}`, params);
  return r.rows;
}

module.exports = { listActive, findPublicById, findOwnedById, listByRequester, listAllForAdmin, create, updateStatus, softDelete, expireDue };
