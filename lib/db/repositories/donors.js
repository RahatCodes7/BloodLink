...
async function searchDonors({ bloodGroup, districtId, upazilaId, availableOnly = true, limit = 20, offset = 0 }) {
  ...
  const r = await query(
    `SELECT ${PUBLIC_DONOR_FIELDS} FROM donor_profiles d WHERE ${w} ORDER BY d.created_at DESC LIMIT $${params.length - 1} OFFSET $${params.length}`, params);
...
module.exports = { upsertProfile, searchDonors, matchDonors };
