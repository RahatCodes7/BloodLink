...
/** নিজের প্রোফাইল আপডেট (availability / দান রেকর্ড)। */
async function updateMine(userId, { availability, lastDonationDate, donationCount }) {
  const sets = [];
  const params = [userId];
  if (availability) { params.push(availability); sets.push(`availability_status=$${params.length}`); }
  if (lastDonationDate) { params.push(lastDonationDate); sets.push(`last_donation_date=$${params.length}`); }
  if (donationCount !== undefined && donationCount !== null) { params.push(donationCount); sets.push(`donation_count=$${params.length}`); }
  if (!sets.length) return null;
  const r = await query(`UPDATE donor_profiles SET ${sets.join(', ')}, updated_at=now() WHERE user_id=$1 RETURNING *`, params);
  return r.rows[0] || null;
}

module.exports = { upsertProfile, searchDonors, matchDonors, updateDonor, updateMine };
