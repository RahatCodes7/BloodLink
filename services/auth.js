...
/** পাসওয়ার্ড রিসেট: Email OTP যাচাই হয়ে থাকলে নতুন hash বসে। */
async function resetPasswordWithEmail({ email, code, newPassword }) {
  if (!newPassword || newPassword.length < 6) { const e = new Error('নতুন পাসওয়ার্ড কমপক্ষে ৬ অক্ষরের দিন।'); e.status = 400; throw e; }
  const { consumeOtp } = require('./otpEmail');
  const { email: em } = await consumeOtp(email, code, 'reset');
  const { query } = require('../lib/db/client');
  const row = (await query('SELECT id FROM users WHERE lower(email)=lower($1) AND deleted_at IS NULL', [em])).rows[0];
  if (!row) { const e = new Error('অ্যাকাউন্ট পাওয়া যায়নি।'); e.status = 404; throw e; }
  const hash = await hashPassword(newPassword);
  await query('UPDATE users SET password_hash=$2, updated_at=now() WHERE id=$1', [row.id, hash]);
  return { id: row.id };
}

module.exports = { COOKIE, TTL, hashPassword, signToken, verifyToken, cookieHeader, clearCookieHeader, registerUser, loginUser, sessionFromToken, resetPasswordWithEmail };
