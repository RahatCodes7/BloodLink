...
import { createdBn, expiryBn } from '@/lib/requestMeta';
  ...
  return (
    <Card className="card-hover">
      ...
      <div className="flex items-center justify-between mt-1.5 text-[11px] text-gray-400">
        <span>📝 {createdBn(r.created_at)}-এ করা</span>
        {exp && !exp.gone && <span className={exp.soon ? 'text-orange-600 font-bold' : ''}>⏳ {exp.text}</span>}
      </div>
